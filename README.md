...
>We need to create a users table and link it with url_shortener table using foreign key
>So we will need schema migrations for this , let's use Alembic
>pip install alembic
>alembic init migrations
>configure alembic.ini file to point to your database
>use the synchronus driver as psycopg2 because alembic operates in synchronous mode by default
>sqlalchemy.url = postgresql+psycopg2://username:password@localhost/dbname
>migrations/env.py find target_metadat variable and point it to SQLAlchemy Base.metadata
>Generate a migration script-->
>alembic revision --autogenerate -m "Details for changes"
>Go to migrations/version ,check if the autogenerated schema changes are as desired. Modify as required.
>alembic upgrade head to commit the new chnages to database
>Rollback if necessary using alembic downgrade -1
>As the table already contains millions of rows so can keep the  user_id column null for earlier enteries or populate it after creating enough users.
>Use soft deletes for when a short code is requested for deletion.
>Seeds the users table with few users
>Run it separately for local environment as a standalone file
>the file is located inside db folder which is one down from root 
>$env:PYTHONPATH = (Get-Location)
>$env:PYTHONPATH temporarily sets the PYTHONPATH environment variable to include current root directory path(e.g. url-shortener/) for it's module search, so that if anything imported from up level will not give error, then simply run python db/filename.py
